...
class SurplusProdusen extends StatefulWidget {
  @override
  _SurplusProdusenState createState() => _SurplusProdusenState();
}

class _SurplusProdusenState extends State<SurplusProdusen> {
  ...
  void _hitung() {
    ...
    integral *= h;
    double surplus = qe * pe - integral;

    setState(() {
      hasil =
          'PS = Qₑ × Pₑ - ∫Ps(Q)dQ\n\n'
          'PS = ${formatAngka(qe)} × ${formatAngka(pe)} - ${formatAngka(integral)}\n\n'
          'PS = ${formatAngka(surplus)}';
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color.fromARGB(255, 154, 250, 154),
      appBar: AppBar(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            Row(
              children: [
                Expanded(child: _button('HITUNG', _hitung, const Color.fromARGB(255, 41, 253, 48))),
                const SizedBox(width: 10),
                Expanded(child: _button('RESET', _reset, const Color.fromARGB(255, 255, 42, 27))),
              ],
            ),
            const SizedBox(height: 20),
            Container(
